...
14/10/2015
- Updated with last commits by Khalid(14 October 2015)
*/
...
//#include "modules\src\antisprintmacro.inc" // anti sprint-macro by shendlaw
...
public OnPlayerConnect(playerid)
{
	...
	{
	    SendClientMessageToAll(-1, sprintf(""COL_PRIM"Version checker: {FFFFFF}the version used in this server is out-dated."));
        SendClientMessageToAll(-1, sprintf(""COL_PRIM"You can visit "COL_SEC"%s "COL_PRIM"to get the latest version", GM_WEBSITE));
    ...
    SendClientMessage(playerid, -1, ""COL_PRIM"Get started: {FFFFFF}/help"COL_PRIM", {FFFFFF}/cmds"COL_PRIM" & {FFFFFF}/changelog");
    SendClientMessage(playerid, -1, ""COL_PRIM"This is an edited version by {FFFFFF}RBK Brigades "COL_PRIM"led by {FFFFFF}[RBK]shendlaw");
    SendClientMessage(playerid, -1, ""COL_PRIM"Download this pro-edit of the gamemode here: {FFFFFF}"GM_WEBSITE"");
    SendClientMessage(playerid, -1, ""COL_PRIM"Download the orignal version: {FFFFFF}"ORIGNAL_GM_WEBSITE"");
    SendClientMessage(playerid, -1, ""COL_PRIM"Original credits: {FFFFFF}Whitetiger"COL_PRIM" & {FFFFFF}[KHK]Khalid"COL_PRIM"");
    new str[128];
...
public OnPlayerRequestClass(playerid, classid)
{
	...
	SetPlayerPos(playerid, -39.256900, 54.445415, 6.484375);
	SetPlayerFacingAngle(playerid, 342.073425);
	SetPlayerCameraPos(playerid, -39.256900 + (5 * floatsin(-342.073425, degrees)), 54.445415 + (5 * floatcos(-342.073425, degrees)), 6.484375);
	SetPlayerCameraLookAt(playerid, -39.256900, 54.445415, 6.484375);
	    ...
	    if(Player[playerid][Playing] == true)
		{
		    switch(Player[playerid][Team])
		    {
		        case ATTACKER:
				{
					...
					format(str, sizeof(str), "~g~~h~%d", TempDamage[ATTACKER]);
...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
	if(dialogid == DIALOG_RADIO)
    {
        if(response)
        {
            switch(listitem)
            {
				case 0: return PlayAudioStreamForPlayer(playerid, "http://www.radio-hit.ro/asculta.m3u");
				case 1: return PlayAudioStreamForPlayer(playerid, "http://www.radiowish.ro/live.m3u");
				case 2: return PlayAudioStreamForPlayer(playerid, "http://radioromanian.net/hiphop.pls");
				case 3: return PlayAudioStreamForPlayer(playerid, "http://radioromanian.net/popular.pls");
				case 4: return PlayAudioStreamForPlayer(playerid, "http://radioromanian.net/manele.pls");
				case 5: return PlayAudioStreamForPlayer(playerid, "http://stream.profm.ro:8012/profm.mp3");
				case 6: return PlayAudioStreamForPlayer(playerid, "http://www.radiogmusic.com/rock.m3u");
				case 7: return PlayAudioStreamForPlayer(playerid, "http://s4.radiohost.pl:8154/listen.pls");
				case 8: return StopAudioStreamForPlayer(playerid);
            }
        }
	    return 1;
	}
	...
	if(dialogid == DIALOG_ATT_SCORE)
	{
	    if(response)
		{
			...
			if(!IsNumeric(inputtext))
			{
				...
				format(iString, sizeof(iString), ""COL_PRIM"Enter {44FF33}%s "COL_PRIM"Team Score Below:", TeamName[ATTACKER]);
			    ShowPlayerDialog(playerid, DIALOG_ATT_SCORE, DIALOG_STYLE_INPUT,""COL_PRIM"Attacker Team Score",iString,"Next","Close");
				return 1;
			}
...
YCMD:radio(playerid, params[], help)
{
    if(help)
	{
	    SendCommandHelpMessage(playerid, "play some music");
	    return 1;
	}
	new RadioString[1024];
    strcat(RadioString, "Radio HiT\n");
    strcat(RadioString, "Radio WiSH\n");
    strcat(RadioString, "Radio Hip-Hop (ROU)\n");
    strcat(RadioString, "Radio Popular (ROU)\n");
    strcat(RadioString, "Radio Manele (ROU)\n");
    strcat(RadioString, "Pro FM\n");
    strcat(RadioString, "Radio GMusic Rock\n");
    strcat(RadioString, "Radio Tube Dubstep PL\n");
    strcat(RadioString, ""COL_PRIM"Turn Off Radio");
    ShowPlayerDialog(playerid, DIALOG_RADIO, DIALOG_STYLE_LIST, ""COL_PRIM"Radio", RadioString, "OK","Close");
    return 1;
}
...
YCMD:checkversion(playerid, params[])
{
	...
	sprintf(""COL_PRIM"- "COL_SEC"Server version:\t\t %s\n"COL_PRIM"- "COL_SEC"Newest version:\t\t %s", GM_NAME, LatestVersionStr), "OK", "");
	return 1;
}
...
YCMD:changelog(playerid, params[], help)
{
	if(help)
	{
	    SendCommandHelpMessage(playerid, "display some changelogs");
	    return 1;
	}
    new ChangelogString[1024];
	strcat(ChangelogString, ""COL_PRIM"13/10/2015 Changelog:");
	strcat(ChangelogString, "\n"COL_PRIM"- "COL_SEC"Made an important update for AC by Khalid.");
	strcat(ChangelogString, "\n"COL_PRIM"- "COL_SEC"Updated with last commits by Khalid (13 October 2015).");
	strcat(ChangelogString, "\n"COL_PRIM"- "COL_SEC"Added some fixes by James Cullum.");
	strcat(ChangelogString, "\n"COL_PRIM"- "COL_SEC"Upgraded to 0.1.13.");
	strcat(ChangelogString, "\n\n"COL_PRIM"Last commits by Khalid: "COL_SEC"13/10/2015");
	strcat(ChangelogString, "\n\n"COL_SEC"Full changelog list: bpf.rbkbrigades.com/changelog.txt");
	strcat(ChangelogString, "\n\n"COL_SEC"What commits is? /commits");
	strcat(ChangelogString, "\n\n"COL_PRIM"Check the current version with: /checkversion.");
	ShowPlayerDialog(playerid,DIALOG_NO_RESPONSE,DIALOG_STYLE_MSGBOX,""COL_PRIM"Server Changelog", ChangelogString, "OK","");
	return 1;
}

YCMD:commits(playerid, params[], help)
{
	if(help)
	{
	    SendCommandHelpMessage(playerid, "What commits is?");
	    return 1;
	}
    new CommitsString[1024];
	strcat(CommitsString, ""COL_PRIM"Commits are all the updates done by "COL_SEC"[KHK]Khalid "COL_PRIM"that you will find only in a newer release of Bulletproof original gamemode.");
    strcat(CommitsString, "\n"COL_PRIM"So using this gamemode you won't wait anymore that "COL_SEC"[KHK]Khalid "COL_PRIM"release a new version of Bulletproof.");
    strcat(CommitsString, "\n\n"COL_PRIM"If you want to check the commits just visit this link: "COL_SEC"http://git.io/v3fQr");
	ShowPlayerDialog(playerid,DIALOG_NO_RESPONSE,DIALOG_STYLE_MSGBOX,""COL_PRIM"Commits", CommitsString, "OK","");
	return 1;
}
...
YCMD:help(playerid, params[], help)
{
    ...
    new HelpString[1024];
	strcat(HelpString, ""COL_PRIM"This is a edit version of Bulletproof led by {FFFFFF}[RBK]shendlaw!");
	strcat(HelpString, "\n"COL_PRIM"Original Credits: {FFFFFF}Whitetiger"COL_PRIM" & {FFFFFF}[KHK]Khalid");
...
YCMD:pchannel(playerid, params[], help)
{
	...
	if(Player[playerid][ChatChannel] != -1) {
		new iString[356];
		iString = "{44FF33}Players in channel:\n\n";

		foreach(new i : Player) {
		    if(Player[i][ChatChannel] == Player[playerid][ChatChannel]) {
		        format(iString, sizeof(iString), "%s{44FF33} - {FFFFFF}%s (%d)\n", iString, Player[i][Name], i);
			}
		}
...
YCMD:admins(playerid, params[], help)
{
	...
	foreach(new i : Player) {
	    if(Player[i][Level] > 0) {
	    	format(iString, sizeof(iString), "%s{FFFFFF}%s ({44FF33}%d{FFFFFF})\n", iString, Player[i][Name], Player[i][Level]);
		}
	}
...
YCMD:minfps(playerid, params[], help)
{
	...
	new iPacket = strval(params);
	if(iPacket < 30 || iPacket > 90) return SendErrorMessage(playerid,"FPS limit can be between 30 and 90 maximum.");
...
YCMD:changepass(playerid, params[], help)
{
    if(help)
	{
	    SendCommandHelpMessage(playerid, "change your user account password.");
	    return 1;
	}
...
public OnPlayerKeyStateChange(playerid, newkeys, oldkeys)
{
	...
	return 1;
	/* Anti Sprint-Macro by shendlaw
	if ((newkeys & KEY_SPRINT) && !(oldkeys & KEY_SPRINT))
 	{
  		if(Player[playerid][Playing] == true)
    	{
       		AntiSprintMacro[playerid] += 1;
            SetTimer("TimerAntiSprintMacro", 1000, false);
         	if(AntiSprintMacro[playerid] == 10) return AnnounceSprintMacroUser(playerid);
		}
	}*/
	return 1;
}
